    ...
    /***/ function (module, __webpack_exports__, __webpack_require__) {
        ...
        return (function recur() {
          ...
          while (!(cur = iter.next()).done) {
            ...
            if (a instanceof Promise) {
              return a.then((a) => ((res.push(a), res).length == l ? res : recur())).catch((e) => (e == nop_0 ? recur() : Promise.reject(e)));
            }
            res.push(a);
        ...
        return (function recur() {
          ...
          while (!(cur = iter.next()).done) {
            ...
            if (b instanceof Promise) {
              return b.then(async (b) => (b ? (res.push(await a), recur()) : res)).catch((e) => (e == nop_0 ? recur() : Promise.reject(e)));
            }
            res.push(a);
          }
          return res;
      ...
      const takeUntil_takeUntil = curry(function takeUntil(f, iter) {
        ...
        return (function recur() {
          ...
          while (!(cur = iter.next()).done) {
            ...
            if (b instanceof Promise) return b.then((b) => (b ? res : recur())).catch((e) => (e == nop_0 ? recur() : Promise.reject(e)));
            if (b) break;
          }
          return res;
        })();
      ...
      const baseCalls = (map, object) =>
        function calls(fs, ...args) {
          return isIterable(fs) ? map((f) => f(...args), fs) : object(Lazy_mapEntriesLazy((f) => f(...args), entriesLazy(fs)));
        };
      ...
      /* harmony default export */ var Concurrency_reduceC = curry(function reduceC(f, acc, iter) {
        return arguments.length == 2 ? reduce(f, Concurrency_catchNoop([...acc])) : reduce(f, acc, Concurrency_catchNoop([...iter]));
      });
